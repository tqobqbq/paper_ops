...
> **Vendored for paper_ops.** This copy lives at `vendor/paper-search-cli/` inside the [paper_ops](../../) repository and has been modified — it does **not** mirror upstream. Do not `git pull` from the original `dr-dumpling/paper-search-cli` repo. See `paper_ops/.agents/skills/paper-ops/` for how it is invoked.
...
- **Funnel-style fallback download**: `download_with_fallback` tries native source download, discovered PDF URLs, PMC/Europe PMC/CORE/OpenAIRE, Unpaywall DOI resolution, then a final DOI-targeted fallback stage.
...
| DOI-targeted lookup | Sci-Hub | DOI-based retrieval; serves as the final automatic PDF fallback |
...
`download_paper` also routes failed or unsupported platform downloads through the same funnel.
